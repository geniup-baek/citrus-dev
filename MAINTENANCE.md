...
이 앱을 유지보수하기 전에 반드시 이해해야 하는 구조적 전제입니다. **로그인(Firebase Auth 이메일/비밀번호)이 필수**이고, **공개 농장 개념이 없습니다** — 농장은 항상 정확히 1명의 소유자를 갖습니다. 로그인 안 된 상태에서 뜨는 화면은 `LoginScreen.vue`뿐이고, `App.vue`가 로그인 여부를 가장 먼저 가릅니다(로그인 전엔 `farms`/`shared`/`sharedCache` 등 어떤 구독도 시도하지 않습니다 — 규칙이 전부 `request.auth != null`을 요구해서 시도해도 거부되기 때문).

- **`farms/{farmId}`와 그 하위(`data/*`)는 소유권 + 구성원 권한으로 막혀 있습니다.** `farms/{farmId}` 문서에 `ownerUid`가 그대로 있습니다(예전엔 노출 방지를 위해 `private/owner` 서브문서로 분리했었지만, 공개 농장이 없어지고 로그인이 필수가 되면서 `get`/`list` 자체를 소유자·구성원·슈퍼관리자로만 제한할 수 있어 서브문서가 필요 없어졌습니다). 소유자·슈퍼관리자(`users/{uid}.role == 'super_admin'`)·그리고 그 농장의 `members/{uid}` 문서에 도메인별(재배동·작업·문제·재고 등) 권한이 있는 구성원만 접근할 수 있습니다. 농장 목록은 `farmsStore.js`가 `where('ownerUid','==',내 uid)` 쿼리(슈퍼관리자는 필터 없이 전체)로 직접 조회합니다 — Firestore가 "쿼리 결과 전체에 대해 규칙이 항상 성립함을 증명"할 수 있어야 `list`를 허용하는데, 이 필터가 그 조건을 그대로 보장해 줍니다. 구성원으로만 속한(소유 아닌) 농장은 이 쿼리에 안 걸리므로, `farmMembersStore.myFarmIds`에 있는 farmId들을 개별로 `getDoc`해서 합칩니다. 규칙 로직은 `firestore.rules`에, 클라이언트 쪽 같은 판단은 `src/utils/farmAccess.js`(`canReadFarmDomain`/`canWriteFarmDomain`)에 있고 **둘은 반드시 같은 논리를 유지해야 합니다.**
- **농장종사자 초대는 최상위 `inviteCodes/{code}` 문서로 이뤄집니다.** 농장주가 권한을 골라 코드를 발급하면(`src/stores/farmMembersStore.js`), 그 코드를 받은 사람이 농장 선택 화면에서 입력해 `farms/{farmId}/members/{uid}`를 그 권한 그대로 생성합니다. 코드는 농장 PIN과 같은 수준의 신뢰 모델(강한 암호화 대상 아님, 엄밀한 1회용 아님)이고, 이메일을 직접 보내는 기능은 없습니다(백엔드가 없어서 카톡 등으로 직접 전달). 구성원이 자기 농장 목록에서 그 농장을 보려면 `users/{uid}.memberFarmIds`(합류할 때 자기 자신이 적어 넣음)를 로그인 시 한 번 확인해 farmsStore의 접근 가능 목록에 합칩니다.
  - ⚠ **`collectionGroup('members').where('uid','==',내 uid))`로 "내가 어느 농장들의 구성원인지"를 한 번에 찾는 방식을 시도했으나 실측으로 거부됨을 확인했습니다** — members의 `list` 규칙에 있는 `isOwner(farmId)` 항이 문서마다 다른 `farmId`(경로)에 의존해서, Firestore가 컬렉션그룹 쿼리(농장이 고정 안 됨)의 "결과 전체 증명 가능"을 판단할 때 그 항 때문에 전체를 증명 불가로 봅니다(다른 항이 uid로 이미 걸려 있어도 소용없음 — OR의 한쪽이 못 증명되면 전체가 거부됩니다). 그래서 `memberFarmIds`(후보) + 개별 `get()` 검증 방식을 그대로 씁니다. `farms` 컬렉션의 `where('ownerUid','==',...)` 쿼리는 이 문제가 없습니다 — `farms/{farmId}`의 `list` 규칙엔 `isOwner`처럼 문서마다 달라지는 경로-의존 항이 없기 때문입니다.
- **`photos/{id}`는 `farmId`로 농장별 접근을 막습니다.** 저장할 때 그 사진을 올린 농장의 id를 같이 써넣고(`src/stores/farmStore/photos.js`, `backup.js`), 규칙은 `farmId`가 있는 사진만 그 농장(소유자/구성원)으로 접근을 제한합니다. `farmId`가 없는(마이그레이션 전 레거시) 사진은 여전히 전체 공개로 열어둡니다 — 사진 하나가 정확히 몇 개 농장에서 참조되는지 기존 데이터만으로는 확실하지 않은 경우(0개=고아, 2개 이상=중복 참조)가 있어, 그런 애매한 사진은 강제로 귀속시키지 않고 그대로 열어둔 채로 남겨뒀습니다. `where('farmId','==',...)` 조회가 필요한 화면(저장용량 계산 등)은 반드시 이 필터를 써야 합니다 — 필터 없는 `photos` 컬렉션 전체 조회는 규칙이 통째로 거부합니다.
- **`main` 브랜치가 쓰던 구버전(단일 농장) 레거시 경로는 이 브랜치(dev)에 없습니다.** `shared/farmData`·`shared/availablePesticide`·`shared/recommendPrefs`·최상위 `treatments/*`와 그걸 다중 농장 구조로 옮기던 `migrateLegacyIfNeeded()`(`farmsStore.js`)는 전부 지웠습니다 — 이 브랜치가 배포되는 Firebase 프로젝트(dev)는 처음부터 다중 농장 구조로 시작해서 옮길 대상이 태초부터 없습니다. **`main`과 이 브랜치는 서로 다른 Firestore 프로젝트를 쓰므로 이 정리가 안전합니다** — 다른 프로젝트에 이 브랜치를 새로 연결한다면(진짜로 처음부터 시작하는 프로젝트가 아니라면) 이 정리가 여전히 맞는지 다시 확인하세요.
- **농장 PIN은 폐지했고, 시스템관리 PIN만 남았습니다.** 공개 농장이 없어지면서(모든 농장이 로그인 필수) 농장 선택 시 PIN을 입력하던 흐름 자체를 없앴습니다. 시스템 관리 진입 PIN(`src/components/FarmSelectScreen.vue`, `VITE_ADMIN_PIN`)은 그대로 남아 있고, 여전히 **슈퍼관리자 로그인이 선행 조건**입니다(`handleAdminClick`) — PIN만으로는 못 들어갑니다.
- **농장은 항상 소유자가 있습니다.** 새 농장을 만들면(`FarmManagementPanel.vue`, 슈퍼관리자 전용 화면) 만든 사람이 자동으로 소유자가 됩니다. 소유권 이전(다른 계정으로 넘기기)은 슈퍼관리자만 할 수 있고, 소유자 본인도 자기 농장의 `ownerUid`는 못 바꿉니다(`firestore.rules`의 `farms/{farmId}` update 규칙). "공개로 되돌리기" 같은 개념 자체가 없습니다 — 항상 다른 계정으로만 옮깁니다.
...
|---|---|---|---|
| `farmsStore.js` | 전역(농장 목록) | `farms/{farmId}` | 농장 생성/이름변경/로고/삭제(소프트)/복원/소유권 이전, 관리모드 전환. **농장 "데이터"는 다루지 않음**. 농장 문서에 `ownerUid`가 그대로 있음(2장 참고, 공개 농장이 없어 노출 문제가 없음) — 접근 가능한 농장 목록은 `where('ownerUid','==',내 uid)` 쿼리(소유) **+** `farmMembersStore.myFarmIds`에 있는 farmId들을 개별 조회(구성원)해서 합침 |
| `farmMembersStore.js` | 농장 1개(구성원 목록) + 전역(내가 속한 농장) | `inviteCodes/{code}`, `farms/{farmId}/members/{uid}`, `farms/{farmId}/inviteCodeRefs/{code}`, `users/{uid}.memberFarmIds` | 농장종사자 초대 코드 발급/폐기, 구성원별 도메인 권한(`canRead`/`canWrite`) 관리, 내가 속한 농장 목록(`myFarmIds`, 로그인 시점에 후보→실제 검증). 소유자·슈퍼관리자는 구성원 전체 목록을 구독하고, 일반 구성원은 자기 권한 문서만 구독함 |
...
├── stores/
│   ├── farmsStore.js       농장 목록/생성/삭제/소유권 이전, 관리모드 전환
